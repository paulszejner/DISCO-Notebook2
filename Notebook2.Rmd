...
```{r}
# The 3 temperature  and Relative humidity values need to be modified to reflect the seasonality described in the introduction
...
#### Leaf transpiration 
``` {r}
E_v_gs <- (wi-wa)*g #(mol m-2 s-1) # barbour 2004
```

#### Leaf surface water vapor
``` {r}
ws_v_gs <- ((Stomatal_conductance*wi)-E_v_gs*(1-wi/2))/(Stomatal_conductance - E_v_gs/2) #mole fraction Roden (2000), Ball (1987).
```

#### Vapor pressure at leaf surface 
``` {r}
esl_v_gs <- ws_v_gs*press_bar #(kPa) higher pressure  than the atmosphere
```
...
Here we can see the differences in Vapour presure defficit along these three times during the growing season.
